...
def pick_diverse(gp_rows, max_count=20):
    buckets = defaultdict(list)
    for row in gp_rows.values():
        key = (row.color_code.upper(), (row.category or '').upper(), (row.rarity or '').upper())
    ...
    selected = []
    used = set()
    # Round-robin across rarity, category, color
    ...
    # Fill any remaining from any pool
    if len(selected) < max_count:
        for key_names in buckets.values():
            for n in key_names:
                if len(selected) >= max_count:
                    break
                if n not in used:
...
def main() -> None:
    ...
    names = pick_diverse(gp_rows, max_count=20)
    ...
    for idx, name in enumerate(names):
        ...
        if not os.path.exists(base_art_path):
            _ = comfyui_generate(art.image_prompt, seed=777000 + idx, save_path=base_art_path, fetch_bytes=False)
        ...
        print(f"Rendered {gp.rarity} {gp.category} ({gp.color_code}) {name} -> {out_path}")
