...
def pick_action_names(gp_rows, max_count=5):
    # Group ACTION names by color, pick one per color first
    buckets = defaultdict(list)
    for row in gp_rows.values():
        if (row.category or '').upper() == 'ACTION':
            buckets[row.color_code.upper()].append(row.name)
    ...
    for c in colors:
        if len(selected) >= max_count:
            break
        if buckets.get(c):
            selected.append(buckets[c][0])
    # Fill remaining with any actions
    if len(selected) < max_count:
        seen = set(selected)
        for c in colors:
            for n in buckets.get(c, []):
                if n not in seen:
                    selected.append(n)
                    seen.add(n)
                    if len(selected) >= max_count:
...
def main() -> None:
    ...
    names = pick_action_names(gp_rows, max_count=5)
    ...
    for idx, name in enumerate(names):
        ...
        if not os.path.exists(base_art_path):
            _ = comfyui_generate(art.image_prompt, seed=222333444 + idx, save_path=base_art_path, fetch_bytes=False)
        ...
        print(f"Generated ACTION card {name} ({gp.color_code}) -> {out_path}")
